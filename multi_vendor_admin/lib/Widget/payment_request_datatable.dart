...
class PayoutRequestData extends StatefulWidget {
  const PayoutRequestData({Key? key}) : super(key: key);

  @override
  State<PayoutRequestData> createState() => _PayoutRequestDataState();
}

class _PayoutRequestDataState extends State<PayoutRequestData> {
  bool isLoaded = false;
        ...
        children: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              FirebaseFirestore.instance
                  .collection('Cash out')
                  .doc(widget.cashOutModel.uid)
                  .update({'paid': true}).then((value) {
                updateHistory(HistoryModel(
                    message: 'Your cash out has been sent to your bank account',
                    amount: '$currencySymbol${widget.cashOutModel.amount}',
                    paymentSystem: 'Paid',
            ...
            child: const Text('Press to update when payout is successful'),
          ),
          const SizedBox(
            height: 20,
          ),
